...
(COD INT,NOM VARCHAR(50),PRE DECIMAL(8,2),
...
DELIMITER $$;
SELECT * FROM PRODUCTO;
CALL SP_INS_ACT_PROD(3,'POLO PLAZA',23.33,33,2,1);
...
BEGIN
DECLARE C INT;
SET C=(SELECT COUNT(*) FROM PRODUCTO WHERE NOM_PROD=XX);
IF C>0 THEN
	BEGIN
		DELETE FROM PRODUCTO WHERE NOM_PROD=XX;
...
BEGIN
	SELECT *FROM PRODUCTO
	WHERE COD_PROD=COD LIKE CONCAT('%',COD,'%') OR 
		  NOM_PROD=NOM LIKE CONCAT('%',NOM,'%');
END $$
DELIMITER $$;
SELECT * FROM PRODUCTO;
SP_BUS_PROD
...
CALL SP_ACT_PROD(NULL,'TELA PLAZA',50,10,3);
...
BEGIN
	SELECT *FROM categoria
	WHERE COD_CAT LIKE CONCAT('%',COD,'%') OR NOM_CAT LIKE CONCAT('%',NOM,'%');
END $$
DELIMITER $$;
...
/***********-----------------------*/
/*ACTUALIZAR DE LA TABLA MARCA BUSCAR*/ 
DROP PROCEDURE IF EXISTS SP_ACTU_MAR;
DELIMITER $$
CREATE PROCEDURE SP_ACTU_MAR
(COD INT,NOM VARCHAR(60))
BEGIN
	UPDATE MARCA SET NOM_MAR LIKE CONCAT('%',NOM,'%') WHERE COD_MAR LIKE CONCAT('%',COD,'%');
END$$
DELIMITER $$;

CALL SP_ACTU_MAR(2,'PONCHOS');
SELECT * FROM MARCA;
/*.----------------------------------------------*/
/*ELIMINAR
 DE LA TABLA DE MARCA ELIMINAR*/
 
DROP PROCEDURE IF EXISTS SP_ELI_MAR;
DELIMITER $$
CREATE PROCEDURE SP_ELI_MAR
(XX VARCHAR(40))
BEGIN
DECLARE C INT;

SET C=(SELECT COUNT(*) FROM MARCA WHERE NOM_MAR=XX);
IF C>0 THEN
	BEGIN
		DELETE FROM MARCA WHERE NOM_MAR=XX;
		SELECT 'REGISTRO ELIMINADO';
	END;
ELSE
	SELECT 'ERROR NO SE ELIMINO';
...
CALL SP_ELI_MAR('PERU');
SELECT * FROM MARCA;
/*-------------------------------------*/
/*REGISTRE
 DE LA TABLA MARCA*/
 
DROP PROCEDURE IF EXISTS SP_INS_MAR;
DELIMITER $$
CREATE PROCEDURE SP_INS_MAR
(COD INT, NOM VARCHAR(60))
BEGIN
...
CALL SP_INS_MAR(NULL,'PERU');
SELECT * FROM MARCA;
/*------------------------------------------------*/

/*BUSQUE DE LA TABLA MARCA*/
DROP PROCEDURE IF EXISTS SP_BUS_MAR;
DELIMITER $$
CREATE PROCEDURE SP_BUS_MAR
(COD INT,NOM VARCHAR(60))
BEGIN
	SELECT *FROM MARCA
	WHERE COD_MAR LIKE CONCAT('%',COD,'%') OR NOM_MAR LIKE CONCAT('%',NOM,'%');
END $$
DELIMITER $$;

CALL SP_BUS_MAR(NULL,'PLATANITOS');
/****--------------------------------------------------*/
/*ACTUALIZA DE LA TABLA MARCA*/
DROP PROCEDURE IF EXISTS SP_ACT_MAR;
DELIMITER $$
CREATE PROCEDURE SP_ACT_MAR
(COD INT,NOM VARCHAR(60))
BEGIN
	IF COD = 0 THEN
		SELECT "ERROR, NO COLOCASTE EL CODIGO";
